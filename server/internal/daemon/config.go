...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	if e, ok := probe("MULTICA_KIMI_CODE_PATH", "kimi-code", "MULTICA_KIMI_CODE_MODEL"); ok {
		agents["kimi-code"] = e
	}
	if len(agents) == 0 {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor-agent, kimi, kimi-code, or kiro-cli and ensure it is on PATH")
	}
...
var defaultAgentCommandNames = []string{
	...
	"gemini", "pi", "cursor-agent", "copilot", "kimi", "kimi-code", "kiro-cli",
}
